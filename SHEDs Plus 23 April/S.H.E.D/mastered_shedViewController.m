...
@synthesize header_title,shed,mastered_list,deletebtn,strid,rowData,movebtn,appDelegate,followeddate,totaldaysSelected,data3,indexList,shedname,seperatorimg,shedtotalrun,seperatorimg1,shedapplieddays,shedlongestrun,daysarray,result,workTab,lifeTab,shedType,mastered_tableview;
...
- (void)viewDidLoad
{
    ...
    if ([shedType isEqualToString:@"work"])
    {
    ...
    while([results next]) {
        ...
        NSLog(@"Shed type..%@",shed.type);
        if ([shed.type isEqualToString:shedType]){
...
-(IBAction)moveshed:(UIControl *)sender
{
  ...
  //  NSString *masteredStr = [[NSString alloc] initWithFormat:@"NO"];
    ...
    [database open];
    NSString *queryStr=[NSString stringWithFormat:@"SELECT COUNT(shed_id) FROM SHED where isMastered ='NO' and type=\"%@\"",shedType];
    int count = [database intForQuery:[NSString stringWithString:queryStr]];
